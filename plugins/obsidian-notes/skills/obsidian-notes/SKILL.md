---
name: obsidian-notes
description: Obsidian 노트를 마크다운 네이티브로 작성하고, 도해가 필요하면 자립형 SVG 파일로 만들어 임베드하는 방법. 끝난 작업의 기록 노트에 담을 내용과 절 순서도 정함. Obsidian vault에 노트를 만들거나 고칠 때, 계획 문서나 스크립트를 옵시디언에 백업하거나 정리할 때, 개념·흐름·아키텍처를 그림으로 설명해야 할 때, 옵시디언에서 그림이 검게 나오거나 스타일이 안 먹을 때, HTML 문서를 옵시디언으로 옮길 때 사용. "옵시디언에 정리해줘", "노트로 만들어줘", "다이어그램 넣어줘", "그림으로 설명해줘", "vault에 저장", "옵시디언에서 안 보여" 같은 요청에 반드시 적용. 옵시디언은 노트 안의 style 태그를 제거하므로 HTML+CSS를 그대로 붙여넣으면 깨진다 — 이 스킬이 그 함정을 피하는 방법을 담고 있으니, 옵시디언 노트를 다룰 때는 먼저 읽을 것.
---
...
1. 노트의 성격을 정함 — 개념 설명, 절차 기록, 회의 정리, 조사 결과, 끝난 작업의 기록 등
2. 본문을 마크다운으로 작성 (아래 "마크다운 작성" 참고). 끝난 작업의 기록이면 "작업 기록 노트의 구성"을 먼저 따름
...
작업이 끝난 뒤 남기는 노트는 계획 문서를 옮겨 적는 것이 아님. 체크리스트, 결정이 남은 것 표, 착수 전 코드 줄번호 조사는 그 시점의 작업 도구라서 끝난 뒤에는 뺌. 노트를 처음 여는 사람이 한 화면 안에서 무엇이 어떻게 바뀌었는지 알 수 있어야 함.

작업 중에 쓰던 계획 파일을 "백업해 달라"는 요청도 같은 기준으로 처리함. 원문을 그대로 붙이지 말고 아래 구성으로 다시 씀. 원문 전체를 보존해야 하면 별도 파일로 `attachments/`에 두고 노트에서 링크함.

절 순서는 다음을 기본으로 하고, 해당 없는 절은 뺌.

1. **무엇이 바뀌었나** — 전과 후를 세 문장 안에서
2. **흐름** — 요청이나 절차가 지나가는 단계를 한 줄씩
3. **핵심 결정** — 표 하나. 항목, 결정, 이유를 한 칸씩. 착수 전 결정과 구현 중 확정된 것을 합쳐 최종 상태만 적음
4. **데이터** — 이름 규칙, 필드, 인덱스처럼 코드를 보지 않고도 알아야 하는 것
5. **깨지면 안 되는 것** — 외부 계약과 불변식
6. **남은 것** — 이 노트를 다시 열 이유가 되는 항목만
7. **부록** — 실행 스크립트처럼 원문이 필요한 것. 설명 주석은 걷어내고 실행 문장만 남김

분량 감각: 500줄짜리 계획 문서가 이 구성으로는 120줄 안에 들어감. 그보다 길어지면 계획 문서의 내용을 옮겨 적고 있는 것임.
