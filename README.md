...
- `korean-style`
  - 공적인 문서의 어휘와 지나친 압축을 걷어내는 output style plugin
  - 포함 output style
    - `plain-korean`
      - 어휘 판단 기준은 "동료에게 말로 설명할 때 그대로 쓸 단어인가"
      - 압축 판단 기준은 "읽는 사람이 이 문장을 한 번에 이해하는가"
      - 금지어 목록 대신 판단이 어긋난 사례를 둠
  - 포함 스크립트
    - `check_korean.py`
      - 문장 길이와 절의 개수처럼 셀 수 있는 신호를 오류로 잡음
      - 어휘는 사례만 주의로 알려 주고 판단은 읽는 사람에게 맡김
